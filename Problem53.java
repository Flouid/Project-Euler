import org.apache.commons.math3.util.CombinatoricsUtils;

import java.math.BigInteger;

/**
 * There are exactly ten ways of selecting three from five, 12345:
 *
 * 123, 124, 125, 134, 135, 145, 234, 235, 245, and 345
 *
 * In combinatorics, we use the notation, 5C3 = 10.
 *
 * In general, nCr = n! / r!(n−r)! where r ≤ n, n! = n×(n−1)×...×3×2×1, and 0! = 1.
 * It is not until n = 23, that a value exceeds one-million: 23C10 = 1144066.
 *
 * How many, not necessarily distinct, values of  nCr, for 1 ≤ n ≤ 100, are greater than one-million?
 */

public class Problem53 {
    public static void main(String[] args) {
        final double startTime = System.nanoTime();                                 //Starts recording runtime
        int greaterThanMillion = 0;
        for (int n = 1; n <= 100; n++) {
            for (int r = 1; r <= n; r++) {
                if (ProblemXX.factorial(n).divide(ProblemXX.factorial(r).multiply(ProblemXX.factorial(n - r))).compareTo(BigInteger.valueOf(1000000)) == 1) {
                    greaterThanMillion++;
                }
            }
        }
        System.out.println(greaterThanMillion);
        final double duration = (System.nanoTime() - startTime) / 1000000000;       //Calculates runtime in seconds
        System.out.println("found in " + duration + " seconds");                    //Outputs runtime
    }
}
